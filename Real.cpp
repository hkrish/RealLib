...
template void RealApplyArrayFunction(RealFuncArray arfunc, ArrayInterface<Real> &arr,
                                     UserInt userdata);

template void RealApplyArrayFunction(RealFuncArray arfunc, std::valarray<Real> &arr,
                                     UserInt userdata);

template void RealApplyArrayFunction(RealFuncArray arfunc, std::vector<Real> &arr,
                                     UserInt userdata);
...
CreateRealConstant(Pi, pi) CreateRealConstant(Ln2, ln2) CreateUnaryRealFunction(sq)
  CreateUnaryRealFunction(abs) CreateUnaryRealFunction(sqrt)
    CreateUnaryRealFunction(rsqrt) CreateUnaryRealFunction(log)
      CreateUnaryRealFunction(exp) CreateBinaryRealFunction(atan2)
        CreateUnaryRealFunction(tan) CreateUnaryRealFunction(cos)
          CreateUnaryRealFunction(sin) CreateUnaryRealFunction(acos)
            CreateUnaryRealFunction(asin) CreateUnaryRealFunction(atan)
...
bool IsProbableZero(const Encapsulation &r, u32 prec)
{
    ...
    return !r.IsNonZero() &&
           Minus(absEncapsulation(r, 0) << prec, Encapsulation(1.0), 0).IsNegative();
...
Encapsulation MakeGoodEstimate(RealObject *pObj, i32 precision, i32 probzeroprec)
{
    ...
    if (g_WorkingPrecision * 32 <= precision + 32)
        InternalReset(precision / 32 + 3);
    ...
    do {
        try {
            ...
            if (!r.IsValueValid() ||
                r.GetRelativeError() < precision && !IsProbableZero(r, probzeroprec)) {
...
#pragma warning(pop)
        ...
        r.weak_AsDecimal(buffer, lenwanted);
    return buffer;
}
...
std::ostream &operator<<(std::ostream &out, const Real &re)
{
    ...
    if (out.flags() & std::ios::scientific)
        ++pr;
    ...
    // reiteration
    {
        ...
        if (!(out.flags() & std::ios::fixed) || r.weak_lt(1)) {
            if (IsProbableZero(r, pzbits) && !(out.flags() & std::ios::fixed))
                out << "probable zero";
            else
                out << r;
            return out;
