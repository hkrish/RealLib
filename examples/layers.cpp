          ...
          MakeFunction(Logistic, s = coeff * s * (one - s), TYPE coeff(3.75); s = s / 2)
            MakeFunction(Sqrt, s = sqrt(s), s = one + one)
              MakeFunction(Abs, s = abs(s), s = one)
                MakeFunction(Tan, s = tan(one += c),
                             TYPE c(0.001)) //TYPE v = pi<TYPE>(prec)/4)
  MakeFunction(Exp, s = exp(s - one), s = TYPE(0.0)) MakeFunction(
    ASine, s = asin(one -= c),
    TYPE c(0.000099)) //s = v * asin(s), s = one / 2; TYPE v = 3 / pi<TYPE>(prec))
  MakeFunction(ATan, s = atan(one += c),
               TYPE c(0.1)) //v * atan(s), s = one; TYPE v = 4 / pi<TYPE>(prec))
...
#define BENCHALL(function, len, ic0, ic1, ic2, ic3)                                      \
    cout << "Benchmarking " #function " (" << s_##function << "), " << len << " members" \
         << endl;                                                                        \
    BENCH(function<double>(0, len), double, "double:          ", ic0, t0);               \
    BENCH(function<MachineEstimate>(0, len), MachineEstimate, "MachineEstimate: ", ic1,  \
          t1);                                                                           \
    BENCH(function(len), Real, "Real, function:  ", ic3, t2);                            \
                                                                                         \
    cout << setprecision(6);                                                             \
    cout << "time: double " << t0 << " mach " << t1 << " est " << t2 << " real " << t3   \
         << endl                                                                         \
