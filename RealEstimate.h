...
static inline std::ostream &operator<<(std::ostream &os, const Estimate &e);
...
class Estimate
{
  private:
    ...
    void
    CorrectZero(); // 0 should not be used in calculations. Substitute (0, e) with (e, 2e)
  ...
  public:
    ...
    u32 GetPrecision() const { return m_Value.GetPrecision(); }
    ...
    {
        return (
          h - ((h - l) - (*this - l).TruncateNegative(origin)).TruncateNegative(origin));
    }
    Estimate TruncateTo(const Estimate &l, const Estimate &h,
    ...
    bool operator<(const Estimate &rhs) const { return (*this - rhs).IsNegative(); }
    bool operator>(const Estimate &rhs) const { return (*this - rhs).IsPositive(); }
    bool operator!=(const Estimate &rhs) const { return (*this - rhs).IsNonZero(); }
    ...
    bool weak_IsNonZero() const { return true; }
    ...
    bool weak_gt(const Estimate &rhs) const { return rhs.weak_lt(*this); }

    bool weak_le(const Estimate &rhs) const { return !weak_gt(rhs); }
    bool weak_ne(const Estimate &rhs) const { return !weak_eq(rhs); }
    bool weak_ge(const Estimate &rhs) const { return !weak_lt(rhs); }
    ...
    i32 weak_normalize() const { return m_Value.normalize(); }

    // weak conversion
    double weak_AsDouble() const { return m_Value.AsDouble(); }
    // output
    ...
    Estimate weak_Center() { return Estimate(m_Value, ErrorEstimate()); }
    ...
    Estimate operator>>(i32 howmuch) const { return *this << -howmuch; }

    Estimate &operator+=(const Estimate &rhs) { return *this = *this + rhs; }
    Estimate &operator-=(const Estimate &rhs) { return *this = *this - rhs; }
    Estimate &operator*=(const Estimate &rhs) { return *this = *this * rhs; }
    Estimate &operator/=(const Estimate &rhs) { return *this = *this / rhs; }

    Estimate &operator>>=(i32 rhs) { return *this = *this >> rhs; }
    Estimate &operator<<=(i32 rhs) { return *this = *this << rhs; }
    Estimate &operator*=(i32 rhs) { return *this = *this * rhs; }
    Estimate &operator/=(i32 rhs) { return *this = *this / rhs; }
    ...
    friend std::ostream &operator<<(std::ostream &os, const Estimate &e);
};

// shorthands
static inline Estimate operator*(i32 lhs, const Estimate &rhs)
{
    return rhs * lhs;
}
