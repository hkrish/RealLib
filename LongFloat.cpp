...
{
    u32 prec = g_WorkingPrecision;
    return ldexp((double)m_Man(prec - 1), -32) + ldexp((double)m_Man(prec - 2), -64) +
...
LongFloat LongFloat::operator+(const LongFloat &rhs) const
{
    ...
    case Normal:
        ...
        case Normal: {
            ...
            if (m_Exp == rhs.m_Exp)
                carry =
                  AddMantissa(manptr, m_Man.getConstPtr(), rhs.m_Man.getConstPtr(), 0);
            else if (m_Exp > rhs.m_Exp)
                carry = AddMantissa(manptr, m_Man.getConstPtr(), rhs.m_Man.getConstPtr(),
                                    i32_saturated(exp - rhs.m_Exp));
            else
                carry = AddMantissa(manptr, rhs.m_Man.getConstPtr(), m_Man.getConstPtr(),
                                    i32_saturated(exp - m_Exp));
...
LongFloat LongFloat::operator-(const LongFloat &rhs) const
{
    ...
    case Normal:
        ...
        case Normal: {
            ...
            if (m_Exp == rhs.m_Exp) {
                carry =
                  SubMantissa(manptr, m_Man.getConstPtr(), rhs.m_Man.getConstPtr(), 0);
                exp = m_Exp;
                neg = m_Neg;
            } else if (m_Exp > rhs.m_Exp) {
                carry = SubMantissa(manptr, m_Man.getConstPtr(), rhs.m_Man.getConstPtr(),
                                    i32_saturated(exp_type(m_Exp) - rhs.m_Exp));
                exp = m_Exp;
                neg = m_Neg;
            } else {
                carry = SubMantissa(manptr, rhs.m_Man.getConstPtr(), m_Man.getConstPtr(),
                                    i32_saturated(exp_type(rhs.m_Exp) - m_Exp));
                exp = rhs.m_Exp;
                neg = !rhs.m_Neg;
            }
            ...
            if (corr == g_WorkingPrecision)
                return LongFloat(Zero);
            else
                return LongFloat(Normal, neg, man, exp - corr, min(m_Prec, rhs.m_Prec));
        }
        }
    }
    return LongFloat(Nan, false);
...
LongFloat LongFloat::operator*(const LongFloat &rhs) const
{
    ...
    case Normal:
        ...
        case Normal: {
            ...
            u32 carry = MulMantissa(manptr, m_Man.getConstPtr(), rhs.m_Man.getConstPtr(),
                                    g_WorkingPrecision - GetPrecision(), GetPrecision());

            if (carry)
                exp += AdjustForCarry(manptr, carry);

            assert(man[g_WorkingPrecision - 1] != 0);
            return LongFloat(Normal, neg, man, i32_saturated(exp),
                             min(m_Prec, rhs.m_Prec));
        }
        }
    }
...
LongFloat LongFloat::operator/(i32 rhs) const
{
    ...
    u32 *manptr = man.getWritablePtr();
    exp_type exp =
      exp_type(m_Exp) + exp_type(InvScaleMantissa(manptr, m_Man.getConstPtr(), rhs));
...
LongFloat LongFloat::recip() const
{
    ...
    case Normal:
        ...
        if (MultipliedByConvolution(il)) {
            ...
            int e = DivMantissa(aptr, aptr, m_Man.getConstPtr(), is, il,
                                t1.getWritablePtr(), t2.getWritablePtr());
...
LongFloat LongFloat::operator/(const LongFloat &rhs) const
{
    if (MultipliedByConvolution(GetPrecision()) || m_Special != Normal ||
        rhs.m_Special != Normal)
    ...
    Mantissa man, t1, t2;
    int e =
      DivMantissa(man.getWritablePtr(), m_Man.getConstPtr(), rhs.m_Man.getConstPtr(), is,
                  il, t1.getWritablePtr(), t2.getWritablePtr());
    LongFloat lf(LongFloat(Normal, m_Neg != rhs.m_Neg, man,
                           exp_type(m_Exp) - rhs.m_Exp + e, m_Prec));
...
{
    if (m_Special != Normal || m_Exp >= g_WorkingPrecision)
        return *this;
    if (m_Exp <= 0)
        return LongFloat(Zero, m_Neg);
    ...
    while (i <= g_WorkingPrecision)
        man[g_WorkingPrecision - i++] = 0;
...
LongFloat LongFloat::round() const
{
    ...
    if (m_Exp < 0)
        return LongFloat(Zero, m_Neg);
    if (m_Exp == 0)
        return m_Man(g_WorkingPrecision - 1) >= (1u << 31) ? LongFloat(m_Neg ? -1.0 : 1.0)
                                                           : LongFloat(Zero, m_Neg);
...
std::ostream &operator<<(std::ostream &os, const LongFloat &lf)
{
    ...
    int pr = os.precision();
    if (os.flags() & ios_base::scientific || os.flags() & ios_base::fixed)
        ++pr;
    int prall = pr + 20;
    ...
    if (os.flags() & ios_base::fixed && pwr > 1)
        pr += pwr - 1;
    if (!(os.flags() & ios_base::fixed || os.flags() & ios_base::scientific) &&
        pwr <= 0 && pwr > -4)
        pr += -pwr + 1;
    ...
    if (!(os.flags() & ios_base::showpoint || os.flags() & ios_base::fixed ||
    ...
    if (os.flags() & ios_base::scientific || pwrsv != pwr) {
        // GCC doesn't understand _itoa, use sprintf instead
        sprintf(buf + pr + 1, "%c%+d", os.flags() & ios_base::uppercase ? 'E' : 'e',
                pwrsv - 1);
    }
    os << setw(w) << buffer;
