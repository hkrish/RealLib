...
#else
u32 MulMantissaDirect(u32 *man, const u32 *a, const u32 *b, i32 inputstart, i32 inputlen)
#endif
{
    i32 kprec = g_KernelsPrecision;
    u32 carry = 0;
    ...
    for (; i < 0 && k < inputlen; ++i, ++k) {
        ...
        for (j = 0; j <= k; ++j) {
            ...
            w += u;
            if (w < u)
                ++carry; // this is a trick to check for carry in u64s
...
u32 MulMantissa(u32 *restrict man, const u32 *restrict a, const u32 *restrict b,
                i32 inputstart, i32 inputlen)
{
    ...
    for (i = max(0, inputlen - 1 - inputstart); i < prec + inputlen - 1; ++i) {
        man[i - inputlen + 1 + inputstart] =
          u32(bufa[i * 2]) + (u32(bufa[i * 2 + 1]) << 16);
    }
...
// incomplete res.
bool SubManBScaled(u32 *res, const u32 *a, const u32 *b, u32 &amsw, i32 bscale,
                   i32 inputlen, i32 inputstart, i32 aofs)
{
#define combinewords(a, b, bscale)                                                       \
    (bscale == 0 ? b : ((a >> (32 - bscale)) + (b << bscale)))
    int carry = 0;
                ...
                u32 *temp1, u32 *temp2)
{
    ...
    for (int k = 0; k < inputstart; ++k)
        man[k] = 0;

    for (sc = 31; sc >= 0; --sc)
        if (SubManBScaled(temp1, a, b, amsw, sc, inputlen, inputstart, 0))
            break;
    ...
    if (sc < 0) {
        ...
        for (sc = 31; sc >= 0; --sc)
            if (SubManBScaled(temp1, a, b, amsw, sc, inputlen, inputstart, 1))
                break;
