...
static inline double max(double u, double v)
{
    return u < v ? v : u;
}
static inline double min(double u, double v)
{
    return u > v ? v : u;
}
...
class MachineEstimate
{
  ...
  public:
    ...
    static double RoundUp(double v) { return v > 0 ? v * OnePEps : v * OneMEps; }
    //{ return _nextafter(v, plusinf); }
    static double RoundDown(double v) { return v > 0 ? v * OneMEps : v * OnePEps; }
    //{ return _nextafter(v, minusinf); }

    static double RoundToZero(double v) { return v * OneMEps; }
    static double RoundFromZero(double v) { return v * OnePEps; }
  ...
  public:
    ...
    MachineEstimate(const char *val)
      : low(RoundDown(atof(val)))
      , high(RoundUp(atof(val)))
    {
    }
    ...
    i32 GetRelativeError() const
    {
        ...
        if (high == low)
            return I32_MAX;
        else if (frexp((high + low) / (high - low), &e) == 0)
            return I32_MIN;
        else
            return e;
    }
    ...
    MachineEstimate TruncateNegative(const char *origin = "Truncate") const
    {
        if (high < 0)
            ...
            return MachineEstimate(max(low, 0.0), high);
    }
    ...
    MachineEstimate TruncateBelow(double l, const char *origin = "Truncate") const
    {
        if (high < l)
            ...
            return MachineEstimate(max(low, l), high);
    }
    ...
    MachineEstimate TruncateAbove(double h, const char *origin = "Truncate") const
    {
        if (low > h)
            ...
            return MachineEstimate(low, min(high, h));
    }
    ...
    MachineEstimate TruncateTo(const MachineEstimate &l, const MachineEstimate &h,
                               const char *origin = "Truncate") const
    {
        return ((h - l) - (*this - l).TruncateNegative(origin)).TruncateNegative(origin) +
               l;
    }

    // comparisons
    // these come in two flavors, strong (true if real is in relation to rhs)
    bool IsPositive() const { return low > 0; }
    ...
    bool operator<(const MachineEstimate &rhs) const { return high < rhs.low; }
    bool operator>(const MachineEstimate &rhs) const { return low > rhs.high; }
    ...
    bool weak_IsPositive() const { return high > -low; }
    bool weak_IsNegative() const { return low < -high; }
    bool weak_IsNonZero() const { return low == -high; }

    bool weak_lt(const MachineEstimate &rhs) const { return Sum() < rhs.Sum(); }
    bool weak_eq(const MachineEstimate &rhs) const { return Sum() == rhs.Sum(); }
    ...
    MachineEstimate weak_round() const { return floor((Sum() + 1.0) * 0.5); }
    ...
    // weak conversion
    double weak_AsDouble() const { return Sum() * 0.5; }
    // output
...
static inline MachineEstimate recip(const MachineEstimate &arg)
{
    ...
    return MachineEstimate(MachineEstimate::RoundDown(l), MachineEstimate::RoundUp(h));
}
...
static inline MachineEstimate sqrt(const MachineEstimate &arg)
{
    ...
    if (
      !(l >=
        0)) // not the same as (l<0): unordered comparison (i.e. l >= 0 is false if l is a NaN, but l<0 is also false)
        l = 0;
...
static inline MachineEstimate abs(const MachineEstimate &arg)
{
    if (arg.low >= 0)
        ...
        return MachineEstimate(0, max(arg.high, -arg.low));
}
