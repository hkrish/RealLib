...
template<class TYPE>
void fft_realtocomplex(int size, TYPE *restrict weights, int *restrict br,
                       TYPE *restrict a, int wstride)
{
    int size2 = size / 2;
    TYPE pr, pi, mr, mi;
    int i, j;
    ...
    for (i = wstride, j = (size - 1) * wstride; i < size2 * wstride;
         (i += wstride), (j -= wstride)) {
        ...
        a[realindex(br[i])] =
          pr + weights[realindex(i)] * pi + weights[imagindex(i)] * mr;
        a[imagindex(br[i])] =
          mi - weights[realindex(i)] * mr + weights[imagindex(i)] * pi;
        a[realindex(br[j])] =
          pr - weights[realindex(i)] * pi - weights[imagindex(i)] * mr;
        a[imagindex(br[j])] =
          -mi - weights[realindex(i)] * mr + weights[imagindex(i)] * pi;
...
template<class TYPE>
void fft_complextoreal(int size, TYPE *restrict weights, int *restrict br,
                       TYPE *restrict a, int wstride)
{
    int size2 = size / 2;
...
template<class TYPE>
void fft_fwd_ip_rc(int size, TYPE *restrict weights, int *restrict br, TYPE *restrict a,
                   int wstride)
{
...
template<class TYPE>
void fft_inv_ip_cr(int size, TYPE *restrict weights, int *restrict br, TYPE *restrict a,
                   int wstride)
{
