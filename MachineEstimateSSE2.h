...
static inline MachineEstimate operator+(const MachineEstimate &lhs,
                                        const MachineEstimate &rhs);
static inline MachineEstimate operator-(const MachineEstimate &lhs,
                                        const MachineEstimate &rhs);
static inline MachineEstimate operator*(const MachineEstimate &lhs,
                                        const MachineEstimate &rhs);
...
class MachineEstimate
{
  ...
  public:
    ...
    static __m128d
      truehigh; // 0x00000000, 0xffffffff. used in log to get the right comparison result
    ...
    MachineEstimate(double l, double h)
      : interval(_mm_set_pd(l, -h))
    {
    }
    MachineEstimate(__m128d src)
      : interval(src)
    {
    }
    ...
    std::ostream &PrintIntervalWHex(std::ostream &os) const
    {
        ...
        return (os << "(" << d[1] << ", " << -d[0] << "), hex diff " << std::hex
                   << (l[0] - l[1]) << std::dec);
    }
    ...
    __m128d Sum() const
    {
        return _mm_sub_pd(interval, _mm_shuffle_pd(interval, interval, 1));
    }
    // gets the difference, negated in both components of the __m128d
    // (i.e. negate second component to make a proper interval)
    __m128d MinusDiff() const
    {
        return _mm_add_pd(interval, _mm_shuffle_pd(interval, interval, 1));
    }
  ...
  public:
    ...
    MachineEstimate(double v = 0.0)
      : interval(_mm_xor_pd(_mm_set1_pd(v), signmask))
    {
    }
    ...
    u32 GetPrecision() const { return 3; }
    MachineEstimate &SetPrecision(u32 prec) { return *this; }

    // comparisons
    // these come in two flavors, strong (true if real is in relation to rhs)
    bool IsNegative() const { return !!_mm_comigt_sd(interval, zero); } // -high > 0
    bool IsPositive() const { return (-*this).IsNegative(); }
    bool IsNonZero() const { return IsPositive() || IsNegative(); }
    ...
    {
        if (REALLIB_MACHEST_INVALID_CHECK(*this) ||
            _mm_comigt_sd(interval, zero)) // -high > 0 means
            throw DomainException(origin); // provably negative input
        else
            return _mm_max_pd(interval, MachineEstimate::sqrt_corr);
    }

    // removes the part of the approximation that is below a certain lower bound
    MachineEstimate TruncateBelow(const MachineEstimate &l,
                                  const char *origin = "Truncate") const
    {
        __m128d ll(_mm_shuffle_pd(l.interval, l.interval, 1));
        __m128d a(_mm_max_pd(
          interval,
          l.interval)); // adjust the lower side to the largest of this and limit
    ...
    MachineEstimate TruncateBelow(double l, const char *origin = "Truncate") const
    {
        ...
        __m128d a(_mm_max_pd(
          interval, ll)); // adjust the lower side to the largest of this and limit
        __m128d b(_mm_move_sd(a, interval)); // and keep the high part from this
    ...
    MachineEstimate TruncateAbove(const MachineEstimate &h,
                                  const char *origin = "Truncate") const
    {
        __m128d ll(_mm_shuffle_pd(interval, interval, 1));
        __m128d a(_mm_max_sd(
          interval,
          h.interval)); // adjust the higher side to the smallest of this and limit
    ...
    MachineEstimate TruncateAbove(double h, const char *origin = "Truncate") const
    {
        ...
        __m128d a(_mm_max_sd(
          interval, lm)); // adjust the higher side to the smallest of this and limit
        // these are high sides, i.e. negated, hence max instead of min
        if (REALLIB_MACHEST_INVALID_CHECK(*this) || _mm_comilt_sd(lh, ll)) // low > h.high
            throw DomainException(origin);
        else
    ...
    MachineEstimate TruncateTo(double l, double h, const char *origin = "Truncate") const
    {
        ...
        if (REALLIB_MACHEST_INVALID_CHECK(*this) ||
            _mm_comigt_sd(c, d)) // low > high in b
            throw DomainException(origin);
        else
            return b;
    }
    ...
    MachineEstimate TruncateTo(const MachineEstimate &l, const MachineEstimate &h,
                               const char *origin = "Truncate") const
    {
    ...
    bool weak_IsPositive() const { return MachineEstimate(Sum()).IsNegative(); }
    bool weak_IsNegative() const { return MachineEstimate(Sum()).IsPositive(); }
    ...
    bool weak_gt(const MachineEstimate &rhs) const { return rhs.weak_lt(*this); }

    bool weak_le(const MachineEstimate &rhs) const { return !weak_gt(rhs); }
    bool weak_ne(const MachineEstimate &rhs) const { return !weak_eq(rhs); }
    bool weak_ge(const MachineEstimate &rhs) const { return !weak_lt(rhs); }
    ...
    MachineEstimate weak_round() const { return floor(weak_AsDouble() + 0.5); }
    ...
    MachineEstimate operator>>(i32 howmuch) const { return *this << -howmuch; }
    ...
    MachineEstimate &operator>>=(i32 rhs) { return *this = *this >> rhs; }
    MachineEstimate &operator<<=(i32 rhs) { return *this = *this << rhs; }
    MachineEstimate &operator*=(i32 rhs) { return *this = *this * rhs; }
    MachineEstimate &operator/=(i32 rhs) { return *this = *this / rhs; }
    ...
    friend std::ostream &operator<<(std::ostream &os, const MachineEstimate &e);
};
...
static inline __m128d myshuffle(const __m128d a, const __m128d b, int m)
{
    switch (m) {
    case 0:
...
static inline __m128d ivchoice(__m128d a, const __m128d b)
{
...
static inline __m128d ivmul(__m128d a, const __m128d b)
{
    a = _mm_xor_pd(a, _mm_set_pd(-0.0, 0.0));
    a = myshuffle(a, a, _mm_movemask_pd(b));
    a = _mm_mul_pd(a, b);
...
static inline __m128d mulpn(const __m128d a, const __m128d b)
{
...
static inline MachineEstimate operator*(const MachineEstimate &lhs, i32 rhs)
{
     ...
     */
    if (rhs >= 0)
        return MachineEstimate(_mm_mul_pd(lhs.interval, _mm_set1_pd(double(rhs))));
    else
        return -lhs * -rhs;
}
...
static inline MachineEstimate operator/(const MachineEstimate &lhs, i32 rhs)
{
    ...
    if (rhs > 0) {
        ...
        throw DomainException("integer div");
}

// shorthands
static inline MachineEstimate operator*(i32 lhs, const MachineEstimate &rhs)
{
    return rhs * lhs;
}
...
static inline MachineEstimate sqrt(const MachineEstimate &arg)
{
    ...
    b = _mm_max_pd(
      b, MachineEstimate::sqrt_corr); // to get rid of a possible NaN in the low part
...
static inline MachineEstimate sq(const MachineEstimate &arg)
{
    ...
    return MachineEstimate(
      _mm_mul_pd(x.interval, _mm_xor_pd(x.interval, MachineEstimate::signmask)));
}
